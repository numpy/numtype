...
assert_type(np.linalg.multi_dot([[[True]], [[True]]]), npt.NDArray[np.bool])
assert_type(np.linalg.multi_dot([[[1]], [[1]]]), npt.NDArray[np.int_])
# pyright is incorrect here (for no obvious reason at all)
assert_type(np.linalg.multi_dot([[[1.0]], [[1.0]]]), npt.NDArray[np.float64])  # pyright: ignore[reportAssertTypeFailure]
assert_type(np.linalg.multi_dot([[[1j]], [[1j]]]), npt.NDArray[np.complex128])
...
assert_type(np.linalg.matmul([[True]], [[True]]), npt.NDArray[np.bool])
assert_type(np.linalg.matmul([1], [1]), np.int_)
assert_type(np.linalg.matmul([1], [True]), np.int_)
assert_type(np.linalg.matmul([[1]], [[1]]), npt.NDArray[np.int_])
assert_type(np.linalg.matmul([1.0], [1.0]), np.float64)
assert_type(np.linalg.matmul([1.0], [1]), np.float64)
# pyright is incorrect here (for no obvious reason at all)
assert_type(np.linalg.matmul([[1.0]], [[1.0]]), npt.NDArray[np.float64])  # pyright: ignore[reportAssertTypeFailure]
assert_type(np.linalg.matmul([1j], [1j]), np.complex128)
assert_type(np.linalg.matmul([True], [1j]), np.complex128)
assert_type(np.linalg.matmul([[1j]], [[1j]]), npt.NDArray[np.complex128])
